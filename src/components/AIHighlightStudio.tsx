...
export const AIHighlightStudio: React.FC<AIHighlightStudioProps> = ({
  ...
  // Use activeHighlightEvent or fallback to the latest event
  const eventToHighlight = activeHighlightEvent || currentMatch.events[0] || {
    id: 'ev_demo',
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 flex items-center gap-1">
                ...
                AI Automated 30s Replay Engine
              </span>
            ...
            <h2 className="text-xl sm:text-2xl font-display font-bold text-white mt-1">
              AI Video Highlights & Mobile Stream Overlay HUD
            </h2>
            <p className="text-xs text-slate-400">
              When a scorer marks a boundary, wicket, goal, or raid point, SportPulse automatically clips the preceding 30 seconds of stream and generates broadcast-grade commentary.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-emerald-400 border border-emerald-500/30 text-xs font-semibold transition-all disabled:opacity-50"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isGeneratingAI ? 'animate-spin' : ''}`} />
              <span>{isGeneratingAI ? 'Synthesizing Call...' : 'Regenerate Gemini Call'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="relative aspect-video bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center overflow-hidden select-none dark-surface video-canvas">
            ...
            >
              {publishedSuccess ? (
                <>
                  ...
                  <span>Published to Social Feed!</span>
                </>
              ) : (
                <>
