      ...
      {isAdmin ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {onOpenPreferences && (
              <button
                id="btn-portal-open-preferences"
                onClick={onOpenPreferences}
                className="w-full py-2.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-100 font-semibold text-xs flex items-center justify-between border border-slate-700 active:scale-95 transition-all"
              >
                <div className="flex items-center gap-2">
                  <Sliders className="w-4 h-4 text-emerald-400" />
                  <span>Display & Typography</span>
                </div>
                <span className="text-[10px] font-mono-code px-2 py-0.5 rounded bg-slate-950 border border-slate-700 text-emerald-400 uppercase">
                  {currentTheme} • {currentFont}
                </span>
              </button>
            )}

            {/* Privacy & Compliance Actions (App Store & GDPR) */}
            <div className="pt-2 border-t border-slate-800 space-y-1.5">
              <span className="text-[10px] font-mono-code uppercase font-bold text-slate-500 block px-1">
                Data Safety & Legal
              </span>

              {onOpenFirebaseServices && (
                <button
                  onClick={onOpenFirebaseServices}
                  className="w-full py-2 px-3 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 text-xs font-semibold flex items-center justify-between border border-amber-500/20 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <Flame className="w-3.5 h-3.5 text-amber-400 fill-amber-400/20" />
                    <span>Firebase Suite (Auth, FCM, Crashlytics)</span>
                  </div>
                  <span className="font-mono-code text-[10px] bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded">
                    Active
                  </span>
                </button>
              )}
              ...
              {onOpenGdprConsent && (
                <button
                  onClick={onOpenGdprConsent}
                  className="w-full py-2 px-3 rounded-xl bg-slate-950/70 hover:bg-slate-800 text-slate-300 text-xs font-medium flex items-center justify-between border border-slate-800 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                    <span>GDPR & Privacy Preferences</span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {onOpenPrivacyPolicy && (
                  <button
                    ...
                    className="py-1.5 px-2.5 rounded-lg bg-slate-950/70 hover:bg-slate-800 text-slate-400 hover:text-white text-[11px] font-medium flex items-center gap-1.5 border border-slate-800 transition-colors"
                  >
              ...
              {onOpenAccountDeletion && (
                <button
                  ...
                  className="w-full mt-1 py-2 px-3 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-xs font-semibold flex items-center justify-center gap-2 border border-rose-500/20 transition-colors active:scale-95"
                >
